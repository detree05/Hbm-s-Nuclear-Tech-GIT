...
public class BiomeGenDima extends BiomeGenBaseCelestial {

	public static final BiomeGenBase biome = new BiomeGenDima(SpaceConfig.dimaBiome);
	...
	public BiomeGenDima(int id) {
		...
		this.setHeight(new BiomeGenBase.Height(0.1F, 0.667F));

		this.topBlock = ModBlocks.dima_regolith;
		this.fillerBlock = ModBlocks.dima_regolith;
	}
		...
		for(int l1 = 255; l1 >= 0; --l1) {
			...
			if(l1 <= 0 + rand.nextInt(5)) {
				...
				if(block2 != null && block2.getMaterial() != Material.air) {
					if(block2 == ModBlocks.dima_stone) {
						if(k == -1) {
							if(l <= 0) {
								block = null;
								b0 = 0;
								block1 = ModBlocks.dima_stone;
							} else if(l1 >= 59 && l1 <= 64) {
								block = this.topBlock;
