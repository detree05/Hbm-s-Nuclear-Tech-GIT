	...
	private void refreshDynamicBedrockRecipes() {
		...
		for(CelestialBedrockOreType type : CelestialBedrockOre.getAllTypes()) {
			for(BedrockOreGrade grade : BedrockOreGrade.values()) {
				...
				ArcFurnaceRecipe recipe = ArcFurnaceRecipes.getOutput(input, true);
				if(recipe != null && recipe.fluidOutput != null && recipe.fluidOutput.length > 0) {
					Object[] out = new Object[recipe.fluidOutput.length];
					for(int i = 0; i < out.length; i++) {
						out[i] = ItemScraps.create(recipe.fluidOutput[i], true);
					}
					this.recipes.put(new ComparableStack(input).makeSingular(), out);
				}
			}
		}
	}
