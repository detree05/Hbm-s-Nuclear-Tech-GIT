...
public class TileEntityMachineDriveProcessor extends TileEntityMachineBase implements IGUIProvider, IControlReceiver, IEnergyReceiverMK2 {
	...
	public void updateEntity() {
		if(!worldObj.isRemote) {
			...
			if(power < maxPower * 0.75) {
				...
				isProcessing = false;
				status = "";
			} else if(ItemVOTVdrive.getDestination(slots[0]).body == SolarSystem.Body.DMITRIY) {
				isProcessing = false;
				status = EnumChatFormatting.RED + "Dmitriy requires digamma ";
	...
	private void processDrive(boolean process) {
		...
		if(ItemVOTVdrive.getDestination(slots[0]).body == SolarSystem.Body.DMITRIY) return;
